...
path_config = Path("/home/kmitsuzawa/codes/mmd-tst-variable-selection-word-sense-change-analysis/configs/coha.toml")
...
def convert_into_index_cchoa(year_label: int) -> int:
    if year_label == 1810:
        return 1
    elif year_label == 1820:
        raise ValueError("No existence")
    else:
        _year = ((year_label - 10) - 1810) / 10
        return int(_year) + 1
# end def
...
def main(t_pair_analysis_target_year: ty.Tuple[int, int], n_top: int = 10):
    ...
    t_pair_analysis_target = (convert_into_index_cchoa(t_pair_analysis_target_year[0]), convert_into_index_cchoa(t_pair_analysis_target_year[1]))
...
for _t_pair in seq_t_pair_analysis_target_year:
    _d_ranking = main(_t_pair)

    _t_pair_analysis_target = (convert_into_index_cchoa(_t_pair[0]), convert_into_index_cchoa(_t_pair[1]))
...
path_output_json = path_dir_result / "result_10.json"
